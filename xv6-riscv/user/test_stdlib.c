...
// Shared runner for the standard library tests. If total_tr is non-NULL,
// aggregate results into the provided test_result (used by tm2). When total_tr
// is NULL, we still run the suite and print the per-suite summary.
void
run_stdlib_tests(struct test_result *total_tr)
{
...
// Standalone entry point used by the dedicated test_stdlib user program.
int
stdlib_suite_main(void)
{
